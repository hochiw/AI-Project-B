...
class Player:
            ...
            for y in [-1,0,1]:
                if (abs(x+y) != 2) and (x,y) != (0,0):
                    ...
                    if tile:
                        ...
                        if [move[0],move[1]] in data['blocks'] or [move[0],move[1]] in data['pieces'] :
                            continue
                    # check if the successor is within the board
                    if -3 <= move[0] <= 3 and -3 <= move[1] <= 3:
                        dist = hex_distance(move, target_goal)
    ...
    # calculates distance
    def hex_distance(tile,other):
    return (abs(tile[0] - other[0]) +
            abs(tile[0] - other[0] + tile[1] - other[1]) +
            abs(tile[1] - other[1])) / 2

    def create_piece(coor, goal, min_dist):
        tmp = {}
        tmp['coor'] = coor
        tmp['goal'] = goal
        tmp['min_dist'] = min_dist
        return tmp
